...
import org.opentripplanner.routing.core.RideSequence;
...
public class DefaultFareServiceImpl implements FareService, Serializable {
    ...
    private FareAndId getBestFareAndId(FareType fareType, List<Ride> rides,
            Collection<FareRuleSet> fareRules) {
        RideSequence rideSequence = new RideSequence(rides);
        if (!rideSequence.isOneFeed()) {
        ...
        for (FareRuleSet ruleSet : fareRules) {
            ...
            if (!ruleSet.hasAgencyDefined() && !attribute.getId().getAgencyId().equals(rideSequence.getFeedId()))
                continue;
            
            if (ruleSet.matches(rideSequence)) {
